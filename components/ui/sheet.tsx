...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-[var(--color-background)] shadow-xl transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
  ...
  <SheetPortal>
    ...
    >
      ...
      <SheetPrimitive.Close
        className="absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)]"
        aria-label="Close"
      >
        <X className="h-4 w-4" />
      </SheetPrimitive.Close>
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn(
      "text-base font-semibold text-[var(--color-foreground)]",
      className,
    )}
    {...props}
  />
