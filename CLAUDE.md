...
- ✅ Step 8 — Enable Banking **on-device** in `CoreIntegrations/EnableBanking/` (was Vercel proxy; re-decided 2026-06-09): RS256 JWT signer (`SecKeyCreateSignature`, Security framework), Keychain key store (iCloud-synced via `kSecAttrSynchronizable` + `WhenUnlocked`; biometric boundary is the app's Face ID gate — a per-item biometric ACL can't coexist with sync), direct REST client (aspsps/auth/sessions/accounts·balances·transactions), Codable models + `Decimal` helpers. PEM→`SecKey` supports PKCS#1 (test-covered) + PKCS#8 (unwrap, untested). **+24 tests (signer/decoder/helpers/request-builder); no live calls. 215 Core + 15 importer green.** Deferred to Step 9.7: `ASWebAuthenticationSession` auth-redirect UI + the `sync-enablebanking.ts` orchestration (account merge/repoint + FX/categorize/transfers post-process → SwiftData).
